...
class CommonController extends Controller
{
    ...
    function getHomeData(){
        ...
        $trending_products = [];
        $trending_products_results =  Product::where('trending','=',1)->where('status','=',1)->orderBy('id','desc')->take(12)->get();
        foreach ($trending_products_results as $product){
            $trending_products[] = productApiResponse($product);
        }
        $response['trending_products'] = $trending_products;

        $sale_products = [];
        $sale_products_results =  Product::where('sale','=',1)->where('status','=',1)->orderBy('id','desc')->take(12)->get();
        foreach ($sale_products_results as $product){
            $sale_products[] = productApiResponse($product);
        }
        $response['sale_products'] = $sale_products;

        return buildResponse('success',"Data get Successfully.", $response);

    }
    function viewAllFlashDeal(){
        $response = [];

        $sale_products = [];
        $sale_products_results =  Product::where('sale','=',1)->where('status','=',1)->orderBy('id','desc')->paginate(20);
        foreach ($sale_products_results as $product){
            $sale_products[] = productApiResponse($product);
        }
        $sale_products_arr = $sale_products_results->toArray();
        $sale_products_arr['data'] = $sale_products;
        $response['sale_products'] = $sale_products_arr;
        return buildResponse('success',"Data get Successfully.", $response);
    }
}